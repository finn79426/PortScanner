# Port Scanner (Rust)

A high-performance, asynchronous CLI tool designed for network reconnaissance. It enumerates subdomains using Certificate Transparency (CT) logs and performs concurrent port scanning on resolved targets.

Designed as a **proof-of-concept** for handling massive concurrency in Rust using `tokio` and `futures`.

## Features

- **Passive Reconnaissance**: Retrieves subdomains from `crt.sh` (Certificate Transparency logs) to minimize direct interaction with the target initially.
- **Async DNS Resolution**: Utilizes `hickory-resolver` (formerly `trust-dns`) with `TokioAsyncResolver` for non-blocking, high-performance domain resolution.
- **Reachability Check**: Ensures subdomains are resolvable via DNS before launching port scans.
- **Massive Concurrency**:
  - **Stream-based Processing**: Replaces traditional MPSC channels with `futures::stream::StreamExt::buffer_unordered` for efficient, resource-friendly concurrency control.
  - **Parallel Scanning**: Scans the top 100 most common ports for each subdomain concurrently, scaling up to thousands of tasks.
- **Non-blocking I/O**: Built entirely on the `tokio` runtime to manage network operations efficiently without blocking the main thread.

## Usage

```shell
cargo run --release -- github.com
```

### Example Output

```text
Port scan completed in 5.5277996 seconds
--------------------------------------------------
jobs.github.com
        5000: opened

classroom.github.com
        80: opened
        443: opened

www.github.com
        22: opened
        80: opened
        443: opened

skyline.github.com
        80: opened
        443: opened

central.github.com
        80: opened
        443: opened

education.github.com
        80: opened
        443: opened

github.com
        22: opened
        80: opened
        443: opened

t.communication.github.com
        80: opened
        443: opened

pkg.github.com
        80: opened
        443: opened

examadmin-uat.github.com
        80: opened
        443: opened

community.github.com
        80: opened
        443: opened

maintainers.github.com
        80: opened
        443: opened

mailing.github.com
        80: opened
        443: opened

m.communication.github.com
        80: opened
        443: opened

res.communication.github.com
        80: opened
        443: opened

communication.github.com

examregistration-uat-api.github.com
        80: opened
        443: opened

render.github.com
        22: opened
        80: opened
        443: opened

api.mcp.github.com
        80: opened
        443: opened

examregistration-api.github.com
        80: opened
        443: opened

raw.github.com
        80: opened
        443: opened

status.github.com
        80: opened
        443: opened

help.github.com
        80: opened
        443: opened

styleguide.github.com
        80: opened
        443: opened

docs.github.com
        80: opened
        443: opened

brandguide.github.com
        80: opened
        443: opened

gist.github.com
        22: opened
        80: opened
        443: opened

atom-installer.github.com
        80: opened
        443: opened

ws.support.github.com
        80: opened
        443: opened

visualstudio.github.com
        80: opened
        443: opened

enterprise.github.com
        80: opened
        443: opened

emails.github.com
        80: opened
        443: opened
        8080: opened
        8443: opened

examadmin.github.com
        80: opened
        443: opened

garage.github.com
        22: opened
        443: opened

examregistration-uat.github.com
        80: opened
        443: opened

edu.github.com
        80: opened
        443: opened

learn.github.com
        80: opened
        443: opened

vscode-auth.github.com
        80: opened
        443: opened

support.enterprise.github.com
        80: opened
        443: opened

examregistration.github.com
        80: opened
        443: opened

slack.github.com
        80: opened
        443: opened

api.security.github.com
        80: opened
        443: opened

mona-arcade.github.com
        80: opened
        443: opened

f.cloud.github.com
        80: opened
        443: opened

vpn-ca.iad.github.com
        80: opened
        443: opened

smtp.github.com
```
